...
watch(() => props.track && !reduced.value, (enabled) => {
  window.removeEventListener('pointermove', onPointer)
  if (enabled) window.addEventListener('pointermove', onPointer, { passive: true })
})
...
onMounted(() => {
  ...
  if (props.track && !reduced.value) window.addEventListener('pointermove', onPointer, { passive: true })
})
onBeforeUnmount(() => {
  window.removeEventListener('pointermove', onPointer)
...
<style scoped>
.tora { display: inline-block; position: relative; flex-shrink: 0; vertical-align: middle; }
